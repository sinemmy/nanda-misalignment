...
check_requirements() {
    ...
    if [[ ! -f "$project_root/.env" ]]; then
        die ".env file not found. Copy .env.example to .env and add your VAST_AI_API_KEY and SSH_KEY_PATH"
    fi
...
print_next_steps() {
    ...
    # Get SSH key option if available
    local ssh_key_opt=""
    if [[ -n "${SSH_KEY_PATH:-}" ]]; then
        SSH_KEY_PATH="${SSH_KEY_PATH/#\~/$HOME}"
        if [[ -f "$SSH_KEY_PATH" ]]; then
    ...
    if [[ -n "$ssh_key_opt" ]]; then
        echo -e "  ${YELLOW}ssh $ssh_key_opt -p $ssh_port root@$ssh_host${NC}"
    else
        echo -e "  ${YELLOW}ssh -p $ssh_port root@$ssh_host${NC}"
    fi
    echo -e "${GREEN}Or use the wrapper script:${NC}"
    echo -e "  ${YELLOW}./deploy/run_remote.sh $ssh_host $ssh_port ls -la${NC}"
    echo
